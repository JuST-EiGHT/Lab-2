...
- [Задание 2](#задание-2)
  ...
  - [2.6 Анализ правильности решения](#26-анализ-правильности-решения)- [Задание 3](#задание-3)
- [Задание 4](#задание-4)
...
Используем переменную `count` типа `int` - счетчик чисел, удволетворяющих условию, переменную `s` - `String` - которая хранит в себе число `i` в строковом формате,   переменные счетчика цикла `i` и `j` - `int`, переменну. `revers` - `boolean` - флаг для проверки палиндрома, и переменную `flag` - `boolean` - флаг для проверки на соответствие маске.
...
| (флаг для проверки на соответствие маске) | `flag`                 | `boolean`     | 
...
2. 10^7  раз проходим по циклу:
  ...
  - Если условие на палиндром выполнено:
    - Вводим переменную `flag` типа `boolean` для проверки на соответствие маске.
    - Проходимся от второго элемента строки, до препоследнего.
      - Если символ = `1`:
        - То  `flag` = `true`. (условие выполнено)
    - Если условие на соответствие маске выполнено:
